...
# ben-pr-review-gh — Local PR Review (post to GitHub)

Reviews a GitHub Pull Request locally using parallel specialized agents from `<HOME>/.agents/lib/ben-pr-review-base.md`, posts findings as inline review comments with `event="COMMENT"`. Never auto-approves or requests changes — leaves the verdict to humans. Optionally schedules a 2-minute watcher cron via `--watch`.
...
**Read `<HOME>/.agents/lib/ben-pr-review-base.md` and follow Steps 3–6 there**, with these inputs:
...
2. GET LAST REVIEWED SHA. Use --arg login binding:
   ...
   set CYCLE_LAST_REVIEWED_SHA = `printf '%s' "${CYCLE_LAST_REVIEWED_RAW}" | jq --arg login "<BOT_LOGIN>" -r '[.[] | select(.user.login == $login or ((.body // "") | test("Parallel PR Review|Code Review Summary|ben-pr-review-gh")))] | sort_by(.submitted_at) | last | .commit_id // ""'`
...
5. **Read `<HOME>/.agents/lib/ben-pr-review-base.md` and follow Steps 3–6 there**, with:
   - <DIFF_SOURCE> = pr
   - <HEAD_REF> = origin/<HEAD_BRANCH>
